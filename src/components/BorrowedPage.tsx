...
import { Landmark, Save, Calendar, User, DollarSign, FileText, Landmark as BankIcon, Edit3, Trash2, CheckSquare } from 'lucide-react';

interface BorrowedPageProps {
  borrowedItems: LentBorrowedRecord[];
  onAddBorrowed: (name: string, amount: number, date: string, dueDate: string | null, paymentMethod: PaymentMethod, note: string) => Promise<void>;
  onCancelBorrowed: (id: string) => void;
  onRestoreBorrowed: (id: string) => void;
...
export default function BorrowedPage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onAddBorrowed(cleanName, numAmount, date, dueDate || null, paymentMethod, note);
      setName('');
      setAmount('');
      setDueDate('');
      setNote('');
    } catch (err: any) {
      alert("Failed to lodge borrowed record: " + err.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card-premium prism-border p-6 md:p-8 rounded-3xl pb-8">
        <div className="flex items-center space-x-3 mb-6">
          <div className="p-2.5 bg-rose-50 dark:bg-rose-950/30 text-rose-505 dark:text-rose-400 rounded-2xl">
            <BankIcon className="w-5.5 h-5.5" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Borrow Capital from a Friend</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">Record capital you borrow from teammates or lenders as liabilities (obligations)</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g., Ahlam Al Shamsi"
                value={name}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center space-x-1.5 font-sans">
                <DollarSign className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505" />
                <span>Amount Borrowed (AED)</span>
              </label>
              <input
                type="number"
                step="0.01"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center space-x-1.5 font-sans">
                <Calendar className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505" />
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center space-x-1.5 font-sans font-mono">
                <Calendar className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505" />
                <span>Payback Due Date</span>
              </label>
              <input
                type="date"
                value={dueDate}
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center space-x-1.5 font-sans">
                <Landmark className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505" />
                <span>Deposit Account Destination</span>
              </label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 flex items-center space-x-1.5 font-sans">
              <FileText className="w-3.5 h-3.5 text-slate-400 dark:text-slate-505" />
              <span>Memo / Agreement Reference Note</span>
            </label>
            <textarea
              rows={2}
              placeholder="Provide context regarding agreement terms or repayments intervals (optional)"
              value={note}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-4 bg-indigo-650 hover:bg-indigo-700 text-white cursor-pointer rounded-2xl font-bold flex items-center justify-center space-x-2 transition shadow-lg shadow-indigo-650/15 active:scale-[0.99] disabled:opacity-50 font-sans"
          >
            {loading ? (
              ...
              <>
                ...
                <span>Lodge Borrowed Record & Income Voucher</span>
              </>
            )}
          </button>
        </form>
      ...
      {/* Borrowed Records History Table */}
      <div className="glass-card-premium prism-border p-6 rounded-3xl">
        <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider mb-4 flex items-center space-x-2">
          ...
          <span>Active Obligation Liabilities</span>
        </h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800/70 text-slate-400 dark:text-slate-500 uppercase text-[9px] tracking-widest font-semibold font-sans">
                ...
                <th className="py-3 px-2 text-right">Repaid Already</th>
                <th className="py-3 px-2 text-center">Settlement Status</th>
              ...
              {borrowedItems.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-6 text-center text-slate-400 dark:text-slate-504 italic">
                    No active borrowed accounts found.
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={item.id} className={`${isCancelled ? 'line-through text-slate-400 dark:text-slate-506 opacity-60' : 'text-slate-700 dark:text-slate-350 hover:bg-slate-50/50 dark:hover:bg-slate-800/30'} transition`}>
                      ...
                      <td className="py-3.5 px-2 text-right font-mono-data">
                        <span className="font-semibold text-rose-600 dark:text-rose-400">{formatAED(item.repaid)}</span>
                      ...
                      <td className="py-3.5 px-2 text-center">
                        {isCancelled ? (
                          ...
                          <span className={`inline-block px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                            ...
                            {item.status === 'paid' ? 'repaid' : item.status === 'partial' ? 'partially repaid' : 'pending'}
                          </span>
                        )}
                      </td>
                      <td className="py-3.5 px-2 text-right space-x-1.5 whitespace-nowrap">
                        {isCancelled ? (
                          <button
                            onClick={() => onRestoreBorrowed(item.id)}
                          ...
                          <>
                            <button
                              ...
                              className="inline-flex items-center space-x-1 px-2.5 py-1.5 bg-sky-50 dark:bg-sky-950/20 text-sky-700 dark:text-sky-455 hover:bg-sky-100 dark:hover:bg-sky-950/45 rounded-lg text-[10px] font-bold uppercase cursor-pointer transition"
                            ...
                            <button
                              onClick={() => onCancelBorrowed(item.id)}
                              className="inline-flex items-center space-x-1 px-2.5 py-1.5 bg-rose-50 dark:bg-rose-950/20 text-rose-650 dark:text-rose-401 hover:bg-rose-100 dark:hover:bg-rose-950/45 rounded-lg text-[10px] font-bold uppercase cursor-pointer transition"
                              title="Cancel Record"
                            >
